...
from pytrainsim.infrastructure import OCP, RestrictedInfra
from pytrainsim.task import Task


class TrainProtectionSystem:
    def __init__(self, tracks: List[RestrictedInfra], ocps: List[OCP]):
        self.tracks = tracks
        self.ocps = ocps
        self.security_elements: Dict[Union[OCP, RestrictedInfra], SecurityElement] = {}
    ...
    def has_capacity(self, element: Union[RestrictedInfra, OCP]) -> bool:
        return self.security_elements[element].has_capacity()

    def reserve(
        self, element: Union[RestrictedInfra, OCP], task: Task, until: datetime
    ) -> bool:
    ...
    def release(self, element: Union[RestrictedInfra, OCP], task: Task) -> bool:
        self.security_elements[element].remove_reservation(task)
        return True

    def extend_reservation(
        self, element: Union[RestrictedInfra, OCP], task: Task, new_release: datetime
    ) -> bool:
        return self.security_elements[element].extend_reservation(task, new_release)

    def next_available_time(
        self, element: Union[RestrictedInfra, OCP]
    ) -> datetime | None:
